...
import type { Env, ProductWithStock, Product } from '../types';
...
products.get('/', async (c) => {
  try {
    const rows = await query<ProductWithStock>(c.env.DB, `
      SELECT
        ...
        CAST(COALESCE(SUM(sb.quantity), 0) AS INTEGER) as stock_quantity
      FROM products p
...
products.put('/:id', authenticateToken, requireRole('office_assistant'), async (c) => {
  try {
    ...
    if (!name || typeof name !== 'string' || name.trim().length === 0) {
      return c.json({ error: 'Nazov produktu je povinny' }, 400);
    }
    ...
    if (ean) {
      ...
      if (existingProduct) {
        return c.json({ error: 'EAN kod uz pouziva iny produkt' }, 400);
      }
    }

    if (sale_price_cents !== undefined && sale_price_cents !== null) {
      if (typeof sale_price_cents !== 'number' || sale_price_cents <= 0) {
        return c.json({ error: 'Akciova cena musi byt kladne cislo' }, 400);
      }
      if (!sale_expires_at) {
        return c.json({ error: 'Datum vyprsania akcie je povinny' }, 400);
      }
    }
    ...
    const product = await queryOne<Product>(c.env.DB,
      `UPDATE products
    ...
    return c.json({ error: 'Nepodarilo sa aktualizovat produkt' }, 500);
  }
});
...
products.delete('/:id', authenticateToken, requireRole('office_assistant'), async (c) => {
  try {
    ...
    if (product.stock_quantity > 0) {
      return c.json({ error: 'Nie je mozne zmazat produkt s nenulovym stavom skladu' }, 400);
    }

    await query(c.env.DB, 'DELETE FROM products WHERE id = ?', [id]);
    return c.json({ message: 'Produkt bol zmazany' });
  } catch (error) {
    console.error('Delete product error:', error);
    return c.json({ error: 'Nepodarilo sa zmazat produkt' }, 500);
  }
});
...
products.get('/:id/price-preview', async (c) => {
  try {
    ...
    const batches = await query<{ quantity: number; price_cents: number }>(c.env.DB,
    ...
    return c.json({ error: 'Nepodarilo sa vypocitat cenu' }, 500);
  }
});
